...
<article>
    <?php foreach ($posts as $post): ?>
        <form action="index.php#<?php echo $post['title']; ?>" id="<?php echo $post['title']?>" method="post">
            <div class="border border-info p-3 m-3">
                ...
                <div class="voteContainer">
                    <div class="upVotes">
                        ...
                        <?php
                        $queryUpVotesUser = $pdo->query('SELECT * FROM votes WHERE direction="1" AND post_id = :post_id AND username = :username');
                        $queryUpVotesUser->bindParam(':post_id', $post['id'], PDO::PARAM_STR);
                        ...
                        <?php if (isset($_SESSION['user'])): ?>
                            ...
                            <?php endif; ?>
                        <?php else: ?>
                            <i class="fa fa-thumbs-o-up fa-lg fa-pull-righ" aria-hidden="true"></i>
                        <?php endif ; ?>
                    </div>
                    <div class="downVotes">
                        <!-- Count of all upvotes on the post -->
                        ...
                        <?php
                        $queryDownVotesUser = $pdo->query('SELECT * FROM votes WHERE direction="-1" AND post_id = :post_id AND username = :username');
                        $queryDownVotesUser->bindParam(':post_id', $post['id'], PDO::PARAM_STR);
